...
export default function List({ statuses, auth }: { statuses: any; auth: any }) {
    const { delete: remove, recentlySuccessful } = useForm();
    const [modalShow, setModalShow] = useState(false);
    const [deletionId, setDeletionId] = useState(0);

    const destroy = (id: number) => {
        remove(route("status.destroy", { status: id }));
        ...
        >
            ...
            <Main>
                ...
                <div className="flex justify-center md:justify-end items-center">
                    <Button pill className="my-3" href={route("status.create")}>
                        Tambah
                    </Button>
                </div>
                <div className="bg-white overflow-x-auto dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
                    <Table>
                        <Table.Head>
                            ...
                            <Table.HeadCell>Nama Status</Table.HeadCell>
                            <Table.HeadCell>
                                Min. Nilai Untuk Lolos
                            </Table.HeadCell>
                            <Table.HeadCell>Aksi</Table.HeadCell>
                        </Table.Head>
                        <Table.Body>
                            {statuses.data.map((item: any, index: number) => {
                                return (
                                    <Table.Row key={index}>
